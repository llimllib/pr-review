...
async function resolveModel(authStorage: AuthStorage, modelRegistry: ModelRegistry, modelId?: string) {
  if (modelId) {
  ...
  if (available.length === 0) {
    throw new Error("No models available. Set ANTHROPIC_API_KEY or configure pi auth.");
  }
  return available[0];
}
...
export async function runReview(options: ReviewOptions): Promise<void> {
  const { diff, cwd, agentNames, modelId, verbose, additionalContext } = options;
  ...
  if (verbose) {
    process.stderr.write(`\x1b[34m• Using model: ${model.provider}/${model.id}\x1b[0m\n`);
    process.stderr.write(`\x1b[34m• Running agents: ${agentNames.join(", ")}\x1b[0m\n\n`);
  }
