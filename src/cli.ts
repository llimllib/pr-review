  ...
  switch (arg) {
    ...
    case "--agents":
      ...
      for (const name of agentNames) {
        if (!ALL_AGENT_NAMES.includes(name)) {
          console.error(`Unknown agent: ${name}. Available: ${ALL_AGENT_NAMES.join(", ")}`);
          process.exit(1);
        }
      }
      i++;
    ...
    case "--context":
      ...
      if (args[i] === "-") {
        // Read from stdin
        try {
          additionalContext += execSync("cat", { stdio: ["inherit", "pipe", "pipe"] }).toString();
        } catch {
...
runReview({ diff, cwd, agentNames, modelId, verbose, additionalContext }).catch((err) => {
